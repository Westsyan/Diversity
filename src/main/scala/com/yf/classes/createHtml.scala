...
object createHtml {
  ...
  def adonis(dir:String) ={
   var html =
     """
       ...
       | <tbody>
       | 	<tr>
       | 		<td colspan="8">Call:</td>
       | 	</tr>
       | 	<tr>
       | 		<td colspan="8">adonis(formula = as.dist(qiime.data$distmat) ~ qiime.data$map[[opts$category]],      permutations = opts$num_permutations) </td>
       | 	</tr>
       | 	<tr><td colspan="8"></td></tr>
       | 	<tr><td colspan="8">Permutation: free</td></tr>
       | 	<tr><td colspan="8">Number of permutations: 999</td></tr>
       | 	<tr><td colspan="8"></td></tr>
       | 	<tr><td colspan="8">Terms added sequentially (first to last)</td></tr>
       | 	<tr><td colspan="8"></td></tr>
       | 	<tr>
       | 		<td></td>
       | 		<td>Df</td>
       ...
       | 	</tr>
     """.stripMargin
    val file = FileUtils.readLines(new File(dir + "/results/04.Beta/Adonis/bray_curtis/adonis_results.txt")).asScala
    val body = file.drop(10)
    val data1 = body.take(1)
    data1.map{x=>
      val datas = x.split(" ").distinct
      html += "<tr>"
      datas.diff(Array(datas(1))).map{y=>
        html += "<td>" + y + "</td>"
      }
        for(i <- 0 until (9-datas.size)){
          html += "<td></td>"
        }
      html += "</tr>"
    }
    val data2 = body.drop(1).take(1)
    data2.map{x=>
      val datas = x.split(" ").distinct
      html += "<tr>"
      val col2 = datas.diff(Array(datas(1)))
      col2.take(4).map{y=>
        html += "<td>" + y + "</td>"
      }
      html += "<td></td>"
      html += "<td>" + col2.last + "</td>"
      for(i <- 0 until (8-datas.size )){
        html += "<td></td>"
      }
      html += "</tr>"
    }

    val data3 = body.drop(2).take(1)
    data3.map{x=>
      val datas = x.split(" ").distinct
      html += "<tr>"
      val col3 = datas.diff(Array(datas(1)))
      col3.take(3).map{y=>
        html += "<td>" + y + "</td>"
      }
      html += "<td></td><td></td>"
      html += "<td>" + col3.last + "</td>"
      for(i <- 0 until (7-datas.size )){
        html += "<td></td>"
      }
      html += "</tr>"
    }
    ...
    html +=
      """
        | 		<tr><td colspan="8">---</td></tr>
        | 		<tr><td colspan="8">Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1</td></tr>
        |     </tr>
        |   </tbody>
        | </table>
        |</div>
      """.stripMargin
    val htmlpath = dir + "/web/res/04.Beta/Adonis/bray_curtis/adonis_results.html"
    FileUtils.writeStringToFile(new File(htmlpath), html, "UTF-8")
  ...
  def dealwithTable(dir:String,exist:String,dataExist:String) = {
    estimators(dir)
    if(dataExist == "true"){
    ...
    val core = getAllValidFile.getFirstXls(dir,"/results/03.Community_composition/Core")
    toptax50(core,dir,3,"core_table.html")
    if(exist == "true") {
      other("/results/04.Beta/Anosim/bray_curtis/anosim_results.txt", dir, 4, "anosim_results.html")
      adonis(dir)
    }
    println("表格处理成功")
  }
